...
from encoder.model import SpeakerEncoder, SpeakerEncoder_2 ###
# from encoder.model import SpeakerEncoder
# from encoder.model_2 import SpeakerEncoder_2 ###
from encoder.model_2 import SpeakerEncoder_2 as SpeakerEncoder_2_768 ###
...
class inference_class():
    ...
    def load_model(self, weights_fpath: Path, device=None, backward=False, encoder_type='A1'):
        """
        ...
        elif encoder_type == 'A2':
            self._model = SpeakerEncoder_2(self._device, torch.device("cpu"))
        elif encoder_type == 'A2_true':
            self._model = SpeakerEncoder_2_768(self._device, torch.device("cpu"))
        else:
        ...
        #         s3l_flag = np.any(np.isnan(v.detach().cpu().numpy()))
        #         if s3l_flag:
        #             print('Nan', k)
        #         else:
        #             print(k, v)
        ...
        for k, v in self._model.named_parameters():
            if isinstance(v, torch.Tensor):
                ...
                #     print(k, v)
        self._model.eval()
        if backward and 'cuda' in str(self._device):
            self._model.train()
            print('Force Train Mode')
    ...
    def embed_frames_batch(self, frames_batch):
        ...
        frames = frames_batch.to(self._device)
        embed = self._model.forward(frames)
        return embed
    ...
    def embed_utterance(self, wav, using_partials=True, return_partials=False, **kwargs):
        ...
        if max_wave_length >= len(wav):
            ...
            wav = torch.nn.functional.pad(wav, (0, max_wave_length - len(wav)), mode="constant")
        ...
        frames_batch = None
        for s in mel_slices:
            f = frames[s].unsqueeze(0)
            if frames_batch is None:
                frames_batch = f
            else:
                frames_batch = torch.cat((frames_batch, f), dim=0)
